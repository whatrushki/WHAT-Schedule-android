...
class DgtuController(
    ...
    private val accountService: DGTUAccountService,
    ...
    override fun obtainEvent(viewEvent: DgtuEvent) = when (viewEvent) {
        is DgtuEvent.AuthClicked -> auth(viewEvent)
        DgtuEvent.MainOpened -> Unit
        ...
        viewModelScope.launchSafe(
            ...
            val stats = with(response.data) {
                DgtuStudentStatInfo(
                    avgCourse = "%.2f".format(
                        Locale.US,
                        markCountStatistic.sumOf { it.mark * it.count } / count.toFloat())
                        .toFloat(),
    ...
    private fun getNotifications() {
        ...
        viewModelScope.launchSafe(
            ...
            val notifications = response.data.feed.mapIndexed { i, it ->
                Notification(
                    id = i,
                    it.category, it.fullDate,
                    if (it.linkText?.takeIf { it.isNotBlank() } != null && it.link?.takeIf { it.isNotBlank() } != null) it.linkText to it.linkText
                    else null,
        ...
        viewModelScope.launchSafe(
            ...
            val profileInfo = with(response.data) {
                DgtuStudentInfo(
                    name,
                    surname,
                    ScheduleSearch.Group(group.item1, group.item2.toString()),
                    kaf.kafName,
                    fullName,
                    middleName,
                    numRecordBook,
                    ...
                    conditionsEducation,
                    trainingDirection,
                    accountService.generateImageLink(photoLink),
                    online,
                    message,
                    htmlBlock,
                    vkID,
